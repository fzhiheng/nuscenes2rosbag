...
namespace nuscenes2bag {
    ...
    SceneConverter::run(const fs::path &inPath,
        ...
        if (sceneInfoOpt) {
            ...
            // convert wheel
            fs::path wheelPath = inPath / "can_bus" / (sceneName + "_zoe_veh_info.json");
            std::cout << "Got wheel json path:" << wheelPath << std::endl;
            convertWheelDatas(outBag, wheelPath);
        }
    ...
    void
    SceneConverter::convertWheelDatas(rosbag::Bag &outBag, const fs::path &inPath) {
        std::cout << "Retrieving WHEEL data" << std::endl;
        auto wheelDatas = metaDataProvider.getWheelData(inPath);
        std::cout << "End retrieving WHEEL data" << std::endl;
        auto topicName = "/wheel";
